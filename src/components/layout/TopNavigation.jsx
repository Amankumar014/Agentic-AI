    ...
    <div className="bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <div className="flex items-center space-x-2 sm:space-x-3 py-4 flex-shrink-0">
          ...
          <nav className="flex items-center overflow-x-auto scrollbar-hide flex-1 min-w-0 ml-2 sm:ml-0">
            <div className="flex items-center">
              {menuItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => onPageChange(item.id)}
                  className={`flex flex-col items-center px-3 sm:px-5 py-3 sm:py-4 transition-all duration-200 relative flex-shrink-0 ${
                    currentPage === item.id
                      ? 'text-primary-500'
